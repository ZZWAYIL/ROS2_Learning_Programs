...
from tf2_ros import TransformBroadcaster # 导入静态变换广播器
...
class TFBroadcasterNode(Node):
    def __init__(self):
        ...
        self.tfbrosdcaster = TransformBroadcaster(self)
        # 发布动态TF点
    ...
    def publish_tf(self):
        ...
        手眼转换
        发布动态TF点
        从 camera_link 到 bottle_link
        """
        tf = TransformStamped()
        tf.header.frame_id = "camera_link"
        tf.child_frame_id = "bottle_link"
